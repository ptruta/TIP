...
class TypeAnalysis(program: AProgram)(implicit declData: DeclarationData) extends DepthFirstAstVisitor[Unit] with Analysis[TypeData] {
  ...
  def visit(node: AstNode, arg: Unit): Unit = {
    ...
    node match {
      case program: AProgram => // For each function of the program, call visit and generate constraint for the sub-AST
      ...
      case fun: AFunDeclaration => unify(fun, FunctionType(fun.params, fun.stmts.ret.exp)) // <-- unify
      case call: ACallFuncExpr => unify(call.targetFun, FunctionType(call.args, call)) // <-- unify
      case _: AReturnStmt =>
      case rec: ARecord =>
