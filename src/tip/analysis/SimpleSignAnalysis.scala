    ...
    n match {
      case r: CfgStmtNode =>
        r.data match {
          // var declarations
          case varr: AVarStmt =>
            //foldleft iterates through the collection, begins from left to right
            //at starts with first item of the collection

            //s is the stateLattice.Element start item

            //valuelattice.top -> any value

            varr.declIds.foldLeft(s)((s,id) => s + (id -> valuelattice.top))
